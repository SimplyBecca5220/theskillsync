...
import { Clock, HandHelping, MessageSquare } from "lucide-react";
import { useRequests } from "@/hooks/use-store";
...
export default function MentorBoard() {
  ...
  const openRequests = requests.filter((r) => !r.claimedBy);
  const claimedRequests = requests.filter((r) => r.claimedBy);
              ...
              <div className="flex items-start justify-between gap-4 mb-3">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground flex items-center gap-1">
                    ...
                    {formatDistanceToNow(req.createdAt, { addSuffix: true })} · by {req.learnerName}
                  </p>
                </div>
              ...
              {claimedRequests.map((req) => (
                ...
                >
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-xs text-primary font-mono">⬡ Claimed by {req.claimedBy}</span>
                  </div>
